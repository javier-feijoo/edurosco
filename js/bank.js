...
function normalizeDistractors(rawValue) {
  ...
  return {
    all: toCleanList(rawValue.all ?? rawValue.comun ?? rawValue.common),
    starts: toCleanList(rawValue.starts ?? rawValue.empieza),
    contains: toCleanList(rawValue.contains ?? rawValue.contiene)
  };
}
...
export function validateAndNormalizeBank(rawBank) {
  ...
  rawBank.preguntas.forEach((item, index) => {
    ...
    const distractores = normalizeDistractors(item.distractores);
