...
let pendingAutoAdvanceTimeout = null;
...
function bindEvents() {
  ...
  refs.answerOptionButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const index = Number.parseInt(button.dataset.optionIndex || "-1", 10);
...
function startGame() {
  ...
  state.game.correctOptionIndex = -1;
  state.game.selectedOptionIndex = -1;
...
function renderActiveQuestionPanel() {
  ...
  if (!active || active.status !== "pending" || !active.questionData) {
    ...
    refs.mcqOptions.classList.add("hidden");
    refs.answerOptionButtons.forEach((button) => {
      button.textContent = "";
  ...
  const choices = buildCurrentQuestionOptions(active);
  state.game.currentOptions = choices.options;
  state.game.correctOptionIndex = choices.correctIndex;
  ...
  refs.mcqOptions.classList.remove("hidden");
  refs.answerOptionButtons.forEach((button, index) => {
    const option = state.game.currentOptions[index];
...
function buildCurrentQuestionOptions(active) {
  const questionData = active?.questionData || {};
  const correct = String(questionData.respuesta || "").trim();
  const configuredWrong = buildConfiguredDistractors({
    questionData,
    typeKey: questionData.tipoKey,
    letter: active?.letra,
    correctAnswer: correct
  });

  const autoWrongPool = buildDistractorPool({
    correctAnswer: correct,
    typeKey: questionData.tipoKey,
    letter: active?.letra
  });
  const mergedWrongPool = mergeUniqueOptions(configuredWrong, autoWrongPool);
  const pickedWrong = shuffleArray(mergedWrongPool).slice(0, 3);

  if (pickedWrong.length > 0 && pickedWrong.length < 3) {
    let cursor = 0;
    while (pickedWrong.length < 3) {
      pickedWrong.push(pickedWrong[cursor % pickedWrong.length]);
      cursor += 1;
    }
  }

  while (pickedWrong.length < 3) {
    pickedWrong.push(correct);
  }

  const options = [
    { text: correct, correct: true },
    ...pickedWrong.map((text) => ({ text, correct: false }))
  ];

  const randomized = shuffleArray(options);
  const correctIndex = randomized.findIndex((entry) => entry.correct);
  return { options: randomized, correctIndex };
}

function buildConfiguredDistractors({ questionData, typeKey, letter, correctAnswer }) {
  const distractores = questionData?.distractores || {};
  const byType = typeKey === "contains"
    ? (Array.isArray(distractores.contains) ? distractores.contains : [])
    : (Array.isArray(distractores.starts) ? distractores.starts : []);
  const common = Array.isArray(distractores.all) ? distractores.all : [];
  const merged = mergeUniqueOptions(byType, common);
  ...
  return merged.filter((candidate) => {
    ...
    return key && key !== correctNormalized && answerMatchesRule(candidate, typeKey, normalizedLetter);
  });
}
...
function buildDistractorPool({ correctAnswer, typeKey, letter }) {
  const allQuestions = Array.isArray(state.questionBank?.questions) ? state.questionBank.questions : [];
  const correctNormalized = normalizeAnswer(correctAnswer);
  const normalizedLetter = normalizeAnswer(letter);
  const seen = new Set();
  const pool = [];
  ...
  allQuestions.forEach((row) => {
    const answer = String(row?.respuesta || "").trim();
    const key = normalizeAnswer(answer);
    if (!answer || key === correctNormalized || seen.has(key)) {
      return;
    }
    if (!answerMatchesRule(answer, typeKey, normalizedLetter)) {
      return;
    }
    seen.add(key);
    pool.push(answer);
...
function answerMatchesRule(answer, typeKey, normalizedLetter) {
  ...
  if (typeKey === "contains") {
    return normalizedAnswer.includes(normalizedLetter);
  }
...
function onSelectOption(index) {
  ...
  const active = getActiveLetterState();
  const validSelection = Number.isInteger(index)
    && index >= 0
    && index < refs.answerOptionButtons.length
    && index < state.game.currentOptions.length;

  if (!active || active.status !== "pending" || !validSelection || state.game.selectedOptionIndex >= 0) {
    return;
  }

  state.game.selectedOptionIndex = index;
  const isCorrect = index === state.game.correctOptionIndex;
  state.game.answerRevealed = true;
  refs.answerBlock.classList.remove("hidden");
  refs.btnReadAnswer.classList.remove("hidden");
  refs.answerOptionButtons.forEach((button, optionIndex) => {
    button.disabled = true;
    button.classList.toggle("is-selected", optionIndex === index);
    button.classList.toggle("is-correct", optionIndex === state.game.correctOptionIndex);
  ...
  clearPendingAutoAdvance();
  pendingAutoAdvanceTimeout = window.setTimeout(() => {
    pendingAutoAdvanceTimeout = null;
    if (state.game.running && !state.game.blockedByTime) {
      moveToNextPending();
    }
  }, 900);
...
function clearPendingAutoAdvance() {
  if (pendingAutoAdvanceTimeout !== null) {
    window.clearTimeout(pendingAutoAdvanceTimeout);
    pendingAutoAdvanceTimeout = null;
  }
}
...
function syncActionButtons() {
  ...
  const canPlay = state.game.running
    ...
    && active
    && active.status === "pending";
  const selectionLocked = state.game.selectedOptionIndex >= 0;

  refs.btnReveal.disabled = true;
  ...
  refs.btnCorrect.disabled = true;
  refs.btnWrong.disabled = true;
  refs.answerOptionButtons.forEach((button, index) => {
    const hasOption = !!state.game.currentOptions[index];
    button.disabled = !canPlay || selectionLocked || !hasOption;
  });
}
...
function getTeacherSnapshot() {
  ...
  return {
    ...
    selectedOptionIndex: state.game.selectedOptionIndex,
    correctOptionIndex: state.game.correctOptionIndex,
